...
class myEventsUI extends StatefulWidget {
  final tipoUI;
  const myEventsUI({required this.tipoUI});

  @override
  _myEventsUIState createState() => _myEventsUIState();
}

class _myEventsUIState extends State<myEventsUI> {
  ...
  Future<List<dynamic>?> obtenerEventosGuardadas() async {
    User? user = FirebaseAuth.instance.currentUser;
    firestore.collection('users').doc(user?.uid).get().then((value) {
      print(value.data()!['eventosGuardadas']);
      setState(() {
        eventosGuardadas = value.data()!['eventosGuardadas'];
      });
    });
  }

  List<dynamic> listaEventosGuardadas(String accion, String uidEvento) {
    var lista = eventosGuardadas;
  ...
  Future<void> subirFavoritos(String uidEvento) async {
    try {
      ...
      docRef.update({
        'eventosGuardadas': FieldValue.arrayUnion([uidEvento])
      });
      print('Ingreso de informacion exitoso.');
      obtenerEventosGuardadas();
  ...
  Future<void> borrarFavoritos(String UidEvento) async {
    try {
      ...
      docRef.update({
        'eventosGuardadas': FieldValue.arrayRemove([UidEvento])
      });
      print('Ingreso de informacion exitoso.');
      obtenerEventosGuardadas();
      // Una vez actualizada la informacion, se devuelve a InfoUser para mostrar su nueva informacion
    ...
    return (InkWell(
      ...
      onTap: () {
        if (tipo == 'Guardar evento') {
          eventosGuardadas.contains(UidEvento)
  ...
  Widget vistaEventData() {
    ...
    return Container(
        ...
        child: (listaEventos.isEmpty && widget.tipoUI == 'Eventos guardados')
            ? Container(
